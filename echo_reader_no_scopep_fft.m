...
for i = 1 : 32
    ...
    s = whos('Band*');
    if isempty(s) || true  %true to override the else-statement
        As(:,i) = A;
...
annotation('textbox', [0.55, 0.65, 0.3, 0.2], 'String', "Please mark the first interesting peak with a rectangle. Then mark the second relevant peak that is closest to it. Then mark the widht of the first envelope.")
...
rect3 = getrect;
flt3 = t>rect3(1) & t < rect3(1) + rect3(3);
...
for i = 1:4
    flti = tAll > times(end) + delta_t1 * 0.998 & tAll < times(end) + delta_t1 * 1.006; %filter out the next relevant peak
    if numel(tAll(flti)) == 1
        times = [times, tAll(flti)];
        pks = [pks;pksAll(flti)];
    else
...
%% Find alfa
Nfft=2^14; %fft number of elements
figure
for i = 1:numel(pks)
    % filter out the pulse envelopes and do fft
    Fi=fft(Amean(flt3), Nfft);
    A_w(:,i)=abs(Fi); %store the spectra
    flt3 = circshift(flt3,delta_t1*1e-6/Tinterval);
    plot(t(flt3),Amean(flt3))
    hold on
    drawnow
    pause
end
...
% find max amplitude frequency and fit amplitudes
fltFreq = f > freqRange(1) & f < freqRange(2); %make a filter for the intersting range
for i = 1 : size(A_w,2)
    A_w_temp = A_w(:,i);
    A_w_flt(:,i) = A_w_temp(fltFreq); %apply the filter
end
Freq_flt = f(fltFreq); %apply the filter

[Apeak, ixPeak] = max(A_w_flt(:,1)); %Find the max amplitude of the first pulse
fPeak = Freq_flt(ixPeak); %find the frequency at max
